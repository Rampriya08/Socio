  ...
  const fetchFollowers= async()=>{
    try {
      const response = await axios.get(
        `http://localhost:5000/api/user/${userId.username}/followers`
      );
   ...
   const fetchFollowing  = async () => {
     try {
       const response = await axios.get(
         `http://localhost:5000/api/user/${userId.username}/following`
       );
  ...
  const fetchData = async (userId, activeTab) => {
    ...
    try {
      ...
      const responses = await axios.get(
        `http://localhost:5000/api/user/get/${userId.username}`
      );
      ...
      switch (activeTab) {
        case "posts":
          response = await axios.get(
            `http://localhost:5000/api/posts/${userId.username}/posts`
          );
